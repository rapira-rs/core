...
// Rust glue (crates/php_sys/src/plugin_handler.rs).
...
static zend_function *rapira_no_ctor(zend_object *object) {
    ...
    // Same as zend_std_get_constructor's throw path: mark the object so a
    // __destruct on a userland subclass cannot run over a half-built instance.
...
void rapira_register_classes(void) {
    ...
    // After registration on purpose: zend_do_inheritance copies the parent's
    // wiring into any class registered with a parent (Zend/zend_inheritance.c),
    // which would undo an earlier assignment on HttpHandler. On the abstract
    // PluginHandler the assignment is for its userland subclasses, which inherit
    // it the same way.
    rapira_plugin_info_ce->default_object_handlers = &rapira_no_ctor_handlers;
...
ZEND_METHOD(Rapira_Plugin_Http_HttpHandlerConfig, __construct) {
    ...
    // info names the capability this config targets, not the extension serving it.
    zval info;
...
ZEND_METHOD(Rapira_Plugin_Http_HttpHandler, handleRequest) {
    ...
    // The fcc is not stored on the object: the call happens inside this frame, so
    // Z_PARAM_FUNC's lifetime is enough and the class needs no get_gc.
